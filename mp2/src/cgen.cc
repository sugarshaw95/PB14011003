...
{
	ValuePrinter vp=ValuePrinter(*ct_stream);
	op_arr_type str_arr=op_arr_type(INT8,25);
	const_value main_str=const_value(str_arr,"Main.main() returned %d\n\00",false);
	vp.init_constant("main.printout.str",main_str);
	//*ct_stream<<", align1";	//"align1 ??"
	...
	vector<op_type> void_type;
	vector<operand> void_op;
	operand return_op0=operand(i32_type,"tpm.0");
	vp.call(*ct_stream,void_type,"Main_main",true,void_op,return_op0);

#ifndef MP3
	// Get the address of the string "Main_main() returned %d\n" using
	// getelementptr
	operand return_op1=operand(i8ptr_type,"tpm.1");
	int_value zero=int_value(0);
	op_arr_type arr_ptr=op_arr_type(INT8_PTR,25);
	global_value op1=global_value(arr_ptr,"main.printout.str"); 
	vp.getelementptr(*ct_stream,str_arr,op1,zero,zero,return_op1);
	// Call printf with the string address of "Main_main() returned %d\n"
	// and the return value of Main_main() as its arguments
	vector<op_type> printf_type;
	vector<operand> printf_op;
	printf_type.push_back(i8ptr_type);
	printf_type.push_back(vararg_type);	
	printf_op.push_back(return_op1);
	printf_op.push_back(return_op0);
	operand return_op2=operand(i32_type,"tpm.2");
	vp.call(*ct_stream,printf_type,"printf",true,printf_op,return_op2);
	// Insert return 0
...
{
	ValuePrinter vp=ValuePrinter(*(class_table->ct_stream));
	...
	operand void_op=operand();
	void_op.set_type(op_type(VOID));
	vector<op_type> type1;
	vector<operand> op1;
	vp.call(*(class_table->ct_stream),type1,"abort",true,op1,void_op);
	vp.unreachable();
	vp.end_define();*/
...
void method_class::code(CgenEnvironment *env)
{
	...
	//op_type ret_type=env->lookup(get_return_type).get_type();
	//operand 
	//Expression expr=get_expr();
	op_type ret_type;
	vector<operand> args;
	string fun_name,method_name,class_name;
	class_name=env->get_class()->get_name()->get_string();
	method_name=name->get_string();
	fun_name=class_name+"_"+method_name;
	
	string type_str=return_type->get_string();
	
	if(type_str=="Int")
	ret_type=op_type(INT32);
	else if(type_str=="Bool")
	ret_type=op_type(INT1);	
	//现在只有int和bool型,第２阶段需要扩展.
	
	
	
	
	//生成args ,这里暂时还不需要(因为main无参数),先占个坑,暂时不写了
	
	vp.define(ret_type, fun_name, args);
	vp.begin_block("entry");//定义入口
	
	
	
	operand ret_val=expr->code(env);
	vp.ret(ret_val);


	vp.begin_block("abort"); //开始abort block
	operand void_op=operand();
	void_op.set_type(op_type(VOID));
	vector<op_type> type1;
	vector<operand> op1;
	vp.call(*(env->cur_stream),type1,"abort",true,op1,void_op);
	vp.unreachable();
	vp.end_define();//结束定义
...
{ 
	...
	vp.store(*(env->cur_stream),rhs,lhs);
	return rhs;
}
...
{ 
	...
	judge=pred->code(env);
	label t_label,f_label,e_label;
	t_label=env->new_label("true.",false);
	f_label=env->new_label("false.",false);
	e_label=env->new_label("end.",true);
	vp.branch_cond(*(env->cur_stream),judge,t_label,f_label);
	
	vp.begin_block(t_label);
	t_op=then_exp->code(env);
	vp.store(t_op,retval);
	vp.branch_uncond(e_label);//true
	
	vp.begin_block(f_label);
	f_op=else_exp->code(env);
	vp.store(f_op,retval);
	vp.branch_uncond(e_label);//false
	
	vp.begin_block(e_label);//end	
	operand f_val=operand(t_op.get_type(),env->new_name());
	vp.load(*(env->cur_stream),t_op.get_type(),retval,f_val);
...
{ 
	...
	// MORE MEANINGFUL
	ValuePrinter vp=ValuePrinter(*(env->cur_stream));
	label l_label,t_label,f_label;
	l_label=env->new_label("loop.",false);
	t_label=env->new_label("true.",false);
	f_label=env->new_label("false.",true);
	vp.branch_uncond(l_label);                   
	vp.begin_block(l_label);//进入loop
	
	operand judge=pred->code(env);
	vp.branch_cond(judge,t_label,f_label);
	vp.begin_block(t_label);
	body->code(env);  
	vp.branch_uncond(l_label);//true执行body并跳回loop
	
	vp.begin_block(f_label);//false只是开始一个新block
	
	                   
	                   
	
	int_value zero=int_value(0);//返回值0
	return zero;
} 
...
{ 
	...
	{
		cur_expr=body->nth(i);
		cur_op=cur_expr->code(env);
		if(!body->more(i+1))
		op=cur_op;
	} //顺序执行每个expr的code,返回最后一个
...
{ 
	...
	ValuePrinter vp=ValuePrinter(*(env->cur_stream));
	operand result;
	string type=type_decl->get_string();
	if(type=="Int")
	{
		result=operand(op_type(INT32_PTR),env->new_name());
				operand op=init->code(env);
		vp.alloca_mem(*(env->cur_stream),op_type(INT32),result);
		env->add_local(identifier,result);
		if(op.is_empty())//无初始化,int默认初始化为0
		{
			int_value zero=int_value(0);
			vp.store(zero,result);
		}
		else
		{			
			vp.store(op,result);
	...
	{
		result=operand(op_type(INT1_PTR),env->new_name());
		vp.alloca_mem(*(env->cur_stream),op_type(INT1),result);
		env->add_local(identifier,result);
		
		operand op=init->code(env);
		if(op.is_empty())//无初始化,bool默认初始化为false
		{
			vp.store(bool_value(false,true),result);
		}
		else
		{			
			vp.store(op,result);
...
{ 
	...
	op1=e1->code(env);
	op2=e2->code(env);
	result=operand(op_type(INT32),env->new_name());
...
{ 
	...
	operand op1,op2,result;
	label ok_label;
	ok_label=env->new_ok_label();
	op1=e1->code(env);
	op2=e2->code(env);
	operand cmp_res=operand(op_type(INT1),env->new_name());
	vp.icmp(*(env->cur_stream),EQ,op2,int_value(0),cmp_res);
	vp.branch_cond(cmp_res,"abort",ok_label); //div 0 check
...
operand comp_class::code(CgenEnvironment *env) 
{
	...
	operand op1,op2,result;
	op1=e1->code(env);
	op2=bool_value(true,true);
	result=operand(op_type(INT1),env->new_name());
	vp.xor_in(*(env->cur_stream),op1,op2,result);
	return result;
}
...
operand int_const_class::code(CgenEnvironment *env) 
{
	...
	// MORE MEANINGFUL
	//Symbol tok=get_tok();
	//operand op=*env->lookup(token);
	string name=token->get_string();
	int in=atoi(name.c_str());
	int_value op=int_value(in);
	return op;
}
...
operand object_class::code(CgenEnvironment *env) 
{
	...
	ValuePrinter vp=ValuePrinter(*(env->cur_stream));
	operand result=operand(op_type(op.get_type().get_deref_type()),env->new_name());
	vp.load(*(env->cur_stream),op.get_type().get_deref_type(),op,result);//从env中找到symbol对应的内存位置,load拿到值
	return result;
}
